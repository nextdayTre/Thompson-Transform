        ...
        <div style={{ padding: "24px 32px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", marginBottom: "24px" }}>
            {[
              ...
              ["Overhead vs JPEG", `${Math.round(economics.overheadRatio)}×`, economics.overheadRatio > 50 ? "#B84040" : "#B87A2E"],
        ...
        <div style={{ padding: "24px 32px" }}>
          <div style={{ background: "#111", borderRadius: "8px", padding: "24px", border: "1px solid #1F1F1F", marginBottom: "20px" }}>
            ...
            <p style={{ fontSize: "13px", color: "#AAA", lineHeight: 1.7, marginBottom: "16px" }}>
              Raw efficiency never crosses — JPEG always wins on file size. But the value proposition
              isn't about size. It's about what the encoding <em style={{ color: "#E8E4DD" }}>survives</em>.
              Below: overhead ratio at each image resolution.
            </p>
          </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px", fontFamily: "JetBrains Mono" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #2A2A2A" }}>
                  {["Resolution", "WAV size", "JPEG size", "Overhead", "Bits/pixel (WAV)"].map((h) => (
                    <th key={h} style={{ padding: "12px 10px", textAlign: "left", color: "#666", fontWeight: 400, fontSize: "10px", letterSpacing: "0.5px" }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {crossoverImages.map((row) => (
                  <tr key={row.size} style={{ borderBottom: "1px solid #1A1A1A" }}>
                    <td style={{ padding: "10px", color: "#E8E4DD" }}>{row.size}×{row.size}</td>
                    <td style={{ padding: "10px", color: "#3A8C3A" }}>{row.wavMB.toFixed(2)} MB</td>
                    <td style={{ padding: "10px", color: "#B87A2E" }}>{row.jpegMB < 0.01 ? (row.jpegMB * 1024).toFixed(1) + " KB" : row.jpegMB.toFixed(2) + " MB"}</td>
                    <td style={{ padding: "10px", color: row.ratio > 100 ? "#B84040" : row.ratio > 50 ? "#B87A2E" : "#3A8C3A" }}>
                      {Math.round(row.ratio)}×
                    </td>
                    <td style={{ padding: "10px", color: "#888" }}>{Math.round(row.bppWav)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          >
            ...
            <div style={{ fontSize: "13px", color: "#AAA", lineHeight: 1.8 }}>
              <p style={{ marginBottom: "12px" }}>
                The spectral encoding is 50-200× larger than JPEG at every resolution.
                Raw efficiency never crosses. But efficiency isn't the only axis:
              </p>
              <div style={{ paddingLeft: "16px", borderLeft: "2px solid #3A8C3A30", marginBottom: "12px" }}>
                ...
                <p style={{ marginBottom: "6px" }}><span style={{ color: "#3A8C3A" }}>Substrate independence:</span> The WAV survives analog transmission, D/A/D conversion, acoustic propagation, format obsolescence.</p>
                <p style={{ marginBottom: "6px" }}><span style={{ color: "#3A8C3A" }}>Steganographic value:</span> The image hides in plain sight as audio. No special container format needed.</p>
                <p style={{ marginBottom: "0" }}><span style={{ color: "#3A8C3A" }}>Multimodal bridging:</span> The same file IS both a sound and an image. It can travel any audio channel.</p>
              </div>
